...
class DenseNet(nn.Module):
    def __init__(
        ...
        spatial_dims: int,
        in_channels: int,
        out_channels: int = 11,  # Changed to 11 for 11 signals
        ...
        # MRI parameters processing
        self.mri_params_fc = nn.Sequential(
            nn.Linear(3, 64),
            get_act_layer(name=act),
            nn.Linear(64, 128)
        )

        # Feature fusion and final layers
        ...
        flattened_size = in_channels + 128  # image features + mri parameter features
        ...
        self.signal_predictor = nn.Sequential(
            ...
            get_act_layer(name=act),
            nn.Dropout(p=dropout_prob),
            nn.Linear(256, out_channels * 50)  # 11 signals × 50 time points
        )
    ...
    def forward(self, x: torch.Tensor, mri_params: torch.Tensor) -> torch.Tensor:
        # Process images through the dense blocks
        x = self.features(x)
        x = self.fusion_layers(x)

        # Process MRI parameters
        mri_features = self.mri_params_fc(mri_params)

        # Combine image features and MRI parameters
        combined_features = torch.cat([x, mri_features], dim=1)

        # Generate signal predictions
        x = self.signal_predictor(combined_features)
