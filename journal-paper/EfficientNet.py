...
class EfficientNet(nn.Module):
    def __init__(
        ...
        spatial_dims: int = 3,           # Changed to 3 for 3D images
        in_channels: int = 9,            # Changed to 9 for 9 TIFF images
        num_classes: int = 550,          # Changed to 11 * 50 for signal prediction
        ...
        self._bn1 = get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=out_channels)
         ...
         # MRI parameter processing branch
        self.mri_param_fc = nn.Sequential(
            nn.Linear(3, 64),
            Act["memswish"](),
            nn.Linear(64, 128),
            Act["memswish"]()
        ...
        # Combine features and predict signals
        combined_features = out_channels + 128  # image features + mri features
        self.final_layers = nn.Sequential(
            nn.Linear(combined_features, 512),
            Act["memswish"](),
            nn.Dropout(dropout_rate),
            nn.Linear(512, self.num_classes)  # num_classes = 11 * 50
        )
    ...
    def forward(self, inputs: torch.Tensor, mri_params: torch.Tensor):
        """
        Args:
            inputs: input should have spatially N dimensions
                ``(Batch, in_channels, dim_0[, dim_1, ..., dim_N])``, N is defined by `dimensions`.
            mri_params: MRI parameters tensor of shape ``(Batch, 3)``.

        Returns:
            Signal predictions of shape ``(Batch, 11, 50)``.
        """
        # Process images
        ...
        # Generate signals
        x = self.final_layers(combined)
        return x.view(-1, 11, 50)
...
class EfficientNetBN(EfficientNet):
    def __init__(
        self,
        model_name: str,
        pretrained: bool = False,         # Changed default to False for 3D
        progress: bool = True,
        spatial_dims: int = 3,            # Changed to 3
        in_channels: int = 9,             # Changed to 9
        num_classes: int = 550,           # Changed to 11 * 50
        norm: str | tuple = ("batch", {"eps": 1e-3, "momentum": 0.01}),
        adv_prop: bool = False,
    ) -> None:
        """
        Modified EfficientNetBN for MRI signal prediction.
        """
        # block args (keep the same)
        blocks_args_str = [
        ...
        # Pretrained weights are not supported for 3D
